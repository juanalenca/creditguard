  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button onClick={handleExport} className="flex items-center px-4 py-2 bg-gray-800 border border-gray-700 text-gray-300 rounded-lg hover:bg-gray-700 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={regiao} onChange={(e) => { setRegiao(e.target.value); setPage(1); }}
          className="px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none cursor-pointer">
            ...
            <tbody className="divide-y divide-gray-700">
              {loading ? (
                ...
                <tr key={c.id} className="hover:bg-gray-700/40 transition-colors group cursor-pointer" onClick={() => navigate(`/clientes/${c.id}`)}>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-mono text-blue-400">{c.id_contrato}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-200">{c.nome_assessoria}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">
                    <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-900/30 text-blue-400 border border-blue-800/50">{c.regiao}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">
